...
tool-business-mail-block の current release-driven development で使用する Operating Model を定義します。
...
- Existing AGENTS / ADR host-layout and validation rules remain project-specific authority.
- `packageManager: bun@1.3.10` is the Bun version authority; mise must materialize the same version, not create a competing pin.
- standalone source validation and host-layout UI/Playwright validation are distinct evidence scopes.
